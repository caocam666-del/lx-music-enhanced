...
## [2.12.5](https://github.com/caocam666-del/lx-music-enhanced/releases/tag/v2.12.5) - 2026-09-13
...
- 新增 酷狗音乐 / QQ 音乐 歌单扫码导入：设置 → 我的列表 → 歌单导入，扫码登录后选择歌单即可批量导入（网易音乐的导入同步增强，支持二维码登录）
- 新增 Wallpaper Engine 动态壁纸支持：设置 → 外观与主题 → 本机 Wallpaper Engine 壁纸，自动扫描 Steam 创意工坊壁纸库，支持视频 / 图片壁纸直接播放，场景 / Web 壁纸以高清预览呈现；壁纸严格裁剪在软件窗口内
- 新增自定义图片壁纸：支持任意本地图片，按壁纸亮度自动调节遮罩透明度，并提供 背景强度 / 背景模糊 微调
- 新增 切歌过渡（复刻 pure-music）：设置 → 播放设置 → 切歌过渡，四种模式
  *无缝衔接*：曲尾预载下一首，零间隙切换；*淡入淡出*：旧歌淡出后新歌淡入（顺序）；*交叉淡化*：结束前数秒新歌提前接入，等功率交叠混音，从已播位置无缝接管；*智能衔接*：按歌曲结尾能量自动选择衔接方式，自动跳过上一首尾部静音与下一首开头静音
- 新增 界面微调：窗口圆角、卡片透明度、背景透明度、背景强度、背景模糊 滑块即时可调并持久化
- 新增 流光背景（三渐变球慢速漂移）与全窗口玻璃拟态视觉（Luminous Harmonic 设计系统）
- 新增 设置页重组：通用 / 外观与主题 独立分组，设置项支持搜索

### 优化

- 页面切换动效复刻 pure-music：新旧页面并行、按菜单方向滑动过渡，不再闪烁
- 排行榜、歌单、搜索等页面内的 榜单/音源/分类/分页 切换改为就地即时刷新（不再整页重载）
- 列表中当前播放歌曲封面：播放中显示四根交错律动的均衡器条，暂停显示播放三角，点击三角可恢复播放
- 自定义滚动条：滚动时浮现、停止后渐隐
- 播放详情页、播放栏等界面的视觉统一与细节打磨；主窗口在后台时保持全速运行（音频渐变不被节流）
- 开发模式下本机地址绕过系统代理，避免窗口加载偶发超时

### 修复

- 修复 Wallpaper Engine 视频壁纸超出软件窗口边界的问题
- 修复列表当前播放封面的播放三角图标渲染异常（viewBox 错误导致显示为不规则图形）
- 修复切歌过渡的多种异常：淡入后音量卡在极小值、交叉淡化后新歌从头重播、交叠结束后切到错误歌曲、后台运行时过渡冻结等

### 说明

- 场景（scene）类 Wallpaper Engine 壁纸需要 WE 引擎渲染，应用内以高清预览图呈现
- 智能衔接基于播放末段实时能量采样决策，与 pure-music 的全曲 DSP 分析（节拍对齐等）存在能力差异
