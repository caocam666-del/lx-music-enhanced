...
declare global {
  ...
  declare namespace LX {
    ...
    interface AppSetting {
      ...
      /**
       * 侧栏折叠态 (220px ↔ 72px 图标模式)
       */
      'common.asideCollapsed': boolean

      /**
       * 播放栏波浪进度条 (full/mini 使用与 middle 同款 SVG 波浪)
       */
      'common.playBarWave': boolean

      /**
       * 播放栏背景音频律动可视化
       */
      'common.playBarVisualization': boolean
       ...
       * 播放失败时按顺序尝试其他自定义源
       */
      'player.autoSwitchSourceOnError': boolean
      ...
      /**
       * Luminous Harmonic: 播放详情页-歌词模糊化（true=上下渐隐+柔影, false=清晰去模糊）
       */
      'playDetail.isLyricBlur': boolean
      ...
      /**
       * 播放详情页-歌词高亮模式 (true=逐字, false=逐行)
       */
      'playDetail.style.lrcFontMode': boolean
       ...
       * 播放详情页-是否显示黑胶中心圆点
       */
      'playDetail.isShowArtworkCenter': boolean
      /**
       * 播放详情页背景配色来源
       */
       ...
       * 桌面歌词音频可视化样式
       */
      'desktopLyric.audioVisualizationStyle': 'bars' | 'wave' | 'orbit' | 'candle'

      /**
       * 是否在全屏时隐藏歌词
       */
      'desktopLyric.taskbarInteractive': boolean
      ...
      /**
       * 桌面歌词形态（normal = 常规歌词窗 / bar = 左下角歌词条）
       */
      'desktopLyric.mode': 'normal' | 'bar'
