...
// Luminous Harmonic: 更新检查结果 toast (原 utils/update.js 的 "TODO add Notice")
// 窗口右上角浮出, 8s 自动关闭
const showUpdateNotice = (message: Parameters<typeof window.i18n.t>[0], substitutions?: Record<string, string>) => {
  void nextTick(() => {
    tips({
      message: window.i18n.t(message, substitutions),
      autoCloseTime: 8000,
      position: {
        top: 64,
  ...
  const showUpdateModal = (status?: LX.UpdateStatus) => {
    ...
    void versionInfoPromise.then((result) => {
      ...
      if (compareVer(versionInfo.version, result.version) != -1) {
        ...
        // Luminous Harmonic: 检查结果 toast 反馈（原为静默）
        showUpdateNotice('update__is_latest')
        handleShowChangeLog()
        return
      }
      ...
      return getIgnoreVersion().then((ignoreVersion) => {
        ...
        showUpdateNotice('update__new_version_tip', { version: result.version })
        void nextTick(() => {
          versionInfo.showModal = true
