  ...
  const handleSyncList = (event: LX.Sync.SyncMainWindowActions) => {
    ...
    switch (event.action) {
      ...
      case 'close_select_mode':
        ...
        break
      case 'server_status':
        sync.server.status.status = event.data?.status ?? false
        sync.server.status.message = event.data?.message ?? ''
        sync.server.status.address = markRaw(Array.isArray(event.data?.address) ? event.data.address : [])
        sync.server.status.code = event.data?.code ?? ''
        sync.server.status.devices = markRaw(Array.isArray(event.data?.devices) ? event.data.devices : [])
        break
      case 'client_status':
        sync.client.status.status = event.data?.status ?? false
        sync.client.status.message = event.data?.message ?? ''
        sync.client.status.address = markRaw(Array.isArray(event.data?.address) ? event.data.address : [])
