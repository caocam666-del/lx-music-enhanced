...
import { play as lyricPlay, pause as lyricPause } from '@renderer/core/lyric'
...
export default () => {
  ...
  const setProgress = (time: number, maxTime?: number) => {
    ...
    setNowPlayTime(time)
    setCurrentTime(time)
    // Luminous Harmonic: 用户拖动/点击跳转后立即同步歌词引擎（目标时间），
    // 播放中直接同步；暂停中同步行位置后冻结，避免暂停时歌词动画继续推进
    lyricPlay(time)
    if (!isPlay.value) lyricPause()
  }
  ...
  const handlePlaying = () => {
    ...
    if (mediaBuffer.playTime) {
      ...
      setNowPlayTime(playTime)
    } else if (restorePlayTime) {
      setCurrentTime(restorePlayTime)
      setNowPlayTime(restorePlayTime)
      restorePlayTime = 0
    }
    // 歌词同步已由 setProgress 完成（目标时间），此处不再重复，避免双重滚动
  }
  const handleWating = () => {
    startBuffering()
  }
