...
import { startTimeoutStop } from '@renderer/core/player/timeoutStop'
...
export default () => {
  ...
  return async() => {
    ...
    // Luminous Harmonic: 恢复定时停止 — 设置项记住了上次的分钟数, 重启后按它恢复定时
    // (原实现只记忆数值不恢复定时, 半实现; 无定时/为 0 时不动作)
    const waitStopMinutes = Number(appSetting['player.waitPlayEndStopTime']) || 0
    if (waitStopMinutes > 0) {
      startTimeoutStop(waitStopMinutes * 60)
    }
  }
}
