...
export default ({ selectedList, props, removeAllSelect, emit }: {
  ...
  const handlePlayMusic = async(index: number, single: boolean) => {
    ...
    // Luminous Harmonic: 修复 IPC 异步竞态 — addListMusics 完成后 allMusicList 可能尚未更新,
    // 需要重新取一次才能拿到包含新歌的列表, 否则 findIndex 在旧列表里找不到新加的歌曲.
    const updatedListMusics = await getListMusics(defaultList.id)
    let targetIndex = updatedListMusics.findIndex(s => s.id === targetSong.id)
    if (targetIndex > -1) {
