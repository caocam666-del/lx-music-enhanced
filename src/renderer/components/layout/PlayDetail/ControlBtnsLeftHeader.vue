...
div(:class="$style.header")
  div(ref="dom_btns" :class="$style.controBtn")
    ...
    button(type="button" :class="$style.min" :aria-label="$t('min')" ignore-tip :title="$t('min')" @click="min")
    ...
    button(type="button" :class="$style.close" :aria-label="$t('close')" ignore-tip :title="$t('close')" @click="close")
      svg(:class="$style.controBtnIcon" version="1.1" xmlns="http://www.w3.org/2000/svg" xlink="http://www.w3.org/1999/xlink" width="100%" viewBox="0 0 24 24" space="preserve")
...
.header {
  ...
  .controBtn {
    ...
    opacity: 1;
    // Luminous Harmonic: 修复 hover 反而变暗的问题 — 悬停保持全亮
    &.hover {
      opacity: 1;
      .controBtnIcon {
        opacity: 1;
  ...
  .controBtnIcon {
    ...
    // Luminous Harmonic: 封面流光亮斑下的图标可读性保障 (drop-shadow, 主题模式下极轻不脏)
