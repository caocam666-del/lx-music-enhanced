...
export default {
  ...
  mounted() {
    ...
    // Luminous Harmonic: 评论视图化后组件按需挂载（挂载时 show 恒为 true），需主动加载一次
  ...
  methods: {
    setWidth() {
      // Luminous Harmonic: 宽度改由父级 CSS 布局（右栏视图 100%），这里只负责触发 tab 布局重算
      setTimeout(() => {
...
.commentHeader {
  ...
  h3 {
    ...
    .mixin-ellipsis-1();
    line-height: 1.2;
    // Luminous Harmonic: 封面取色模式近白 + 阴影, 主题模式回退主题色无阴影
...
.commentMain {
  flex: auto;
  // Luminous Harmonic: 玻璃 token 底色 (原 --color-primary-light-400-alpha-700 在花哨封面下可读性一般)
  background-color: var(--glass-surface-weak, var(--color-primary-light-400-alpha-700));
