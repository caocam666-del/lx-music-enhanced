  ...
  <material-modal frosted :show="modelValue" bg-close teleport="#view" width="680px" max-height="86%" @close="handleClose">
    <main class="scroll" :class="$style.main">
      ...
      <section v-if="!account" :class="$style.form">
        ...
        <p :class="$style.tip">{{ $t('qq__qr_tip') }}</p>
        <base-btn :class="$style.primaryBtn" :disabled="scanning" @click="handleScanLogin">
          {{ scanning ? $t('qq__qr_waiting_btn') : $t('qq__qr_btn') }}
        </base-btn>
        ...
        <div v-show="showCookieInput" :class="$style.cookieArea">
          <p :class="$style.tip">{{ $t('qq__cookie_tip') }}</p>
          <textarea
            v-model="cookieText" :class="$style.cookieInput"
            :placeholder="$t('qq__cookie_placeholder')" spellcheck="false" autocomplete="off"
          />
      ...
      <section v-else :class="$style.accountSection">
        <div :class="$style.accountBar">
          <span :class="$style.accountName">{{ $t('qishui__account', { name: nickname || 'QQ 音乐' }) }}</span>
        ...
        <div :class="$style.selectionBar">
          <base-checkbox
            id="qq_select_all" :model-value="allSelected" :label="$t('netease__select_all')"
        ...
        <ul v-if="playlists.length" class="scroll" :class="$style.playlists">
          <li v-for="playlist in playlists" :key="playlist.id" :class="$style.playlist">
            <base-checkbox
              :id="`qq_playlist_${playlist.id}`" v-model="selectedIds" :value="playlist.id"
              :label="playlist.name"
            />
        ...
        <p :class="$style.hint">{{ $t('qq__play_hint') }}</p>
      </section>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const handleScanLogin = async() => {
      ...
      scanning.value = true
      scanStatus.value = t('qq__qr_window_open')
      error.value = ''
      try {
        const result = await openQQLoginWindow()
        if (result?.ok && result.cookie) {
          scanStatus.value = t('qq__qr_syncing')
          await setQQCookie(result.cookie)
    ...
    const errorText = err => {
      if (err?.message == 'QQ_AUTH_REQUIRED') return t('qq__auth_required')
      if (err?.message == 'QQ_PLAYLIST_EMPTY') return t('netease__empty')
    ...
    const handleLoadLibrary = async() => {
      ...
      error.value = ''
      try {
        const result = await getQQLibrary()
    ...
    const handleLoad = async() => {
      ...
      status.value = ''
      try {
        await setQQCookie(cookieText.value.trim())
        await handleLoadLibrary()
      } catch (err) {
        error.value = err?.message == 'QQ_AUTH_REQUIRED' ? t('qq__auth_required') : errorText(err)
      } finally {
        loading.value = false
      }
    }
    ...
    const handleImport = async() => {
      ...
      try {
        for (const playlist of selected) {
          ...
          const target = userLists.find(list => list.source === 'tx' && String(list.sourceListId) === playlist.id)
          try {
            if (target) {
              await syncSourceList(target)
            } else {
              const tracks = await getQQPlaylistTracks(playlist.id)
