...
// Luminous Harmonic: 在线取到新封面 URL 后回写下载条目 (原 core/music/download.ts getPicUrl 的 TODO)
export const updateTaskPicUrl = (downloadInfo: LX.Download.ListItem, picUrl: string) => {
  if (!picUrl) return
  downloadInfo.metadata.musicInfo.meta.picUrl = picUrl
  throttleUpdateTask([downloadInfo])
}
