...
export const getNeteaseCookie = async() => {
  return rendererInvoke<string | null>(WIN_MAIN_RENDERER_EVENT_NAME.get_netease_cookie)
}
export const setNeteaseCookie = async(cookie: string) => {
  await rendererInvoke<string, undefined>(WIN_MAIN_RENDERER_EVENT_NAME.set_netease_cookie, cookie)
}
export const clearNeteaseCookie = async() => {
  await rendererInvoke(WIN_MAIN_RENDERER_EVENT_NAME.clear_netease_cookie)
}

// Luminous Harmonic: 汽水音乐（仅歌单导入：扫码登录 + 歌单拉取）
export interface QishuiPlaylistMeta {
  id: string
  name: string
  cover: string
  trackCount: number
...
export interface QishuiLibraryResult {
  userId: string
  nickname: string
  playlists: QishuiPlaylistMeta[]
}
export interface QishuiTrackMeta {
  id: string
  name: string
  singer: string
  album: string
...
export const getQishuiStatus = async() => {
  return rendererInvoke<{ loggedIn: boolean, userId: string }>(WIN_MAIN_RENDERER_EVENT_NAME.qishui_status)
}
export const createQishuiQrCode = async() => {
  return rendererInvoke<{ token: string, qrcode: string, qrcodeIndexUrl: string, scanUrl: string, expireTime: number }>(WIN_MAIN_RENDERER_EVENT_NAME.qishui_qr_create)
}
export const checkQishuiQrConnect = async(token: string) => {
  return rendererInvoke<string, { loggedIn: boolean, status: string, errorCode: number }>(WIN_MAIN_RENDERER_EVENT_NAME.qishui_qr_check, token)
}
export const clearQishuiQr = async() => {
  await rendererInvoke(WIN_MAIN_RENDERER_EVENT_NAME.qishui_qr_clear)
}
export const getQishuiLibrary = async() => {
  return rendererInvoke<QishuiLibraryResult>(WIN_MAIN_RENDERER_EVENT_NAME.qishui_get_library)
}
export const getQishuiPlaylistTracks = async(playlistId: string) => {
...
/**
 * 设置窗口位置与大小 (x/y/width/height 任意子集, 走的是同一个 set_window_size 通道,
 * 主进程 setWindowBounds 接收 Partial<Rectangle>)
 * Luminous Harmonic: 用于 max 按钮 - 因为 BrowserWindow 创建时 maximizable:false,
 * browserWindow.maximize() 能进最大化但 unmaximize() 不响应, 会卡死.
 * 所以改用 setWindowBounds 把窗口直接铺满屏幕 (伪最大化), 还原时再设回原 preset size.
 */
export const setWindowBounds = (bounds: Partial<Electron.Rectangle>) => {
  rendererSend(WIN_MAIN_RENDERER_EVENT_NAME.set_window_size, bounds)
}
