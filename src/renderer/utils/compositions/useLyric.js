...
export default ({ isPlay, lyric, playProgress, isShowLyricProgressSetting, offset }) => {
  ...
  const handleLyricClick = event => {
    if (isMsDown.value || suppressLyricClick || !event.target.closest) return
    const line = event.target.closest('.line-content')
    if (!line || line.time == null) return
    const nextTime = Math.max(line.time - lyric.offset - lyric.tempOffset, 0) / 1000
    time = Math.min(nextTime, playProgress.maxPlayTime)
    isStopScroll.value = false
    window.app_event.setProgress(time)
    if (!isPlay.value) play()
  }
  ...
  const handleReturnCurrentLyric = () => {
    isStopScroll.value = false
    clearLyricScrollTimeout()
    if (cancelScrollFn) {
      cancelScrollFn()
  ...
  const handleScrollLrc = (duration = 700) => {
  ...
  const handleMouseMsUp = event => {
    ...
    if (suppressLyricClick) setTimeout(() => { suppressLyricClick = false }, 0)
  }
  const handleMove = (y) => {
    if (isMsDown.value) {
      if (Math.abs(msDownY - y) > 4) suppressLyricClick = true
  ...
  const setLyric = (lines) => {
    // The out-in transition can temporarily delay the lyric node mount. Keep
    // the latest lines and flush them from the transition completion hook;
    // repeatedly queueing nextTick here can starve the renderer.
    if (!dom_lyric.value || !dom_lyric_text.value) {
      pendingLines = lines.length ? lines : null
      dom_lines = null
      return
    }
    pendingLines = null
  ...
  const handleLyricTransitionEnter = () => {
    if (pendingLines && lyric.lines === pendingLines) setLyric(pendingLines)
  }
  ...
  const initLrc = (lines, oLines) => {
    ...
    if (oLines) {
      if (lines.length) {
        ...
        // The old lyric node may already be gone while the out-in transition
        // is leaving. There is nothing to scroll until the next lyric mounts.
        if (!dom_lyric.value) {
          pendingLines = null
          dom_lines = null
          return
  ...
  const scrollLine = (line, oldLine) => {
    ...
    // Luminous Harmonic: 默认下一句开始唱的同时立即平滑滚动（滚动动画本身 700ms 缓动形成平滑上移）；
    // "延迟歌词滚动"设置开启时恢复 600ms 延迟（当前句唱完片刻再上移）
  ...
  // The lyric node is toggled by hasLyric during a song switch. Run this
  // watcher after Vue has mounted that node so new lyrics are not lost.
  watch(() => lyric.lines, initLrc, { flush: 'post' })
