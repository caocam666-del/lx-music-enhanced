...
import { getNeteaseCookieValue } from '@renderer/utils/neteaseAuth'
...
export default {
  ...
  async getCookie() {
    return (await getNeteaseCookieValue()) || this.cookie
  },
  async requestAccountApi(url, params) {
    const cookie = await this.getCookie()
    if (!cookie || cookie === 'MUSIC_U=') throw new Error('NETEASE_AUTH_REQUIRED')
    ...
    if (statusCode !== 200 || !body || body.code !== this.successCode) {
      if (body?.code === 301 || body?.code === 401) throw new Error('NETEASE_AUTH_REQUIRED')
      throw new Error('NETEASE_REQUEST_FAILED')
    }
    return body
  },
  async getAccount() {
    const body = await this.requestAccountApi('https://music.163.com/api/nuser/account/get', {})
    const account = body.account || {}
    const profile = body.profile || {}
    const uid = account.id || profile.userId
    if (!uid) throw new Error('NETEASE_AUTH_REQUIRED')
    return {
      id: String(uid),
      nickname: profile.nickname || account.userName || '',
    }
  },
  async getUserPlaylists(uid, limit = 1000) {
    const list = []
    let offset = 0
    let total = 0
    do {
      const body = await this.requestAccountApi('https://music.163.com/api/user/playlist', {
        uid,
        limit,
        offset,
        includeVideo: true,
      ...
      total = Number(body.count || playlists.length)
      list.push(...playlists)
  ...
  async getListDetail(rawId, page, tryNum = 0) { // 获取歌曲列表内的音乐
    ...
    else this.cookie = await this.getCookie()
