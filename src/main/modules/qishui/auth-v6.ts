/**
 * 汽水音乐官方 Passport Web 二维码登录（主进程）。
 * 移植自 Mineradio（XxHuberrr/Mineradio，GPL-3.0）qishui-auth-v6.js
 * 及 Wx2yZx/Mineradio-Qishui-QR-Login（GPL-3.0-only），纯自用集成。
 * 仅走官方扫码/轮询/二次验证流程；安全组件资源为字节跳动/汽水所有，
 * 仅用于与用户自己的官方账号会话互通。
 */

/* eslint-disable @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-member-access */
// @ts-nocheck -- 逐字移植的 GPL 上游实现, 保持原样不逐行补类型
...
import { getConfig, updateConfig } from './auth-config'
...
const AUTH_PARTITION = 'persist:lx-qishui-auth-v6'
...
const ASSET_DIR =  false
  ? 0
  : path.join(__dirname, 'qishui-auth')
...
function getCookie() {
  return String(getConfig().cookie || '').trim()
}
...
async function clear() {
  ...
  // eslint-disable-next-line require-atomic-updates -- 单例重置：current 已为快照，此处仅清空引用
  runtime = null
}
