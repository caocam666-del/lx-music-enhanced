...
const fs = require('fs')
const projectTemp = path.join(__dirname, '..', '.tmp')
...
process.env.ELECTRON_BUILDER_CACHE = process.env.ELECTRON_CACHE
...
const options = {
  ...
  directories: {
    ...
    // Luminous Harmonic: 支持环境变量覆盖输出目录 (旧输出目录被占用时, 覆盖安装不受影响)
    output: process.env.BUILD_OUTPUT_DIR || './build',
  },
  files: [
    '!node_modules/**/*',
