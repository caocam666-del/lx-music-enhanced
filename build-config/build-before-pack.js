...
const replaceQrcDecodeLib = async(electronNodeAbi, platform, arch) => {
  ...
  // Luminous Harmonic: Windows Defender / 索引服务常临时锁住 .node 文件, 触发 EPERM。
  // 如果目标文件已存在且与源大小一致, 跳过 copy, 让 dev 继续启动。
  await fsPromises.copyFile(filePath, targetPath).catch((err) => {
    if (err.code === 'EPERM' && fs.existsSync(targetPath)) {
      console.warn('[build-before-pack] qrc_decode.node copyFile EPERM but target exists, skip (likely Defender lock)')
      return
    }
    throw err
  })
